...
│   └── lib/
│       ├── features/
│       │   ├── federated/    # FL client service (FedAvg local training, DP noise, weight upload)
...
- Last update staleness and the resulting FedAsync discount weight
...
docker exec $(docker ps -qf name=redis) redis-cli -a secret --no-auth-warning \
    DEL fl:global_weights fl:round fl:client_ids fl:total_samples fl:last_staleness fl:agg_lock
