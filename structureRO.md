...
- **1.1 Context** - apariția aplicațiilor mobile bazate pe locație pentru
  ...
  privind ML centralizat pe date comportamentale personale; Federated
  Learning (învățare federată, FL) ca alternativă.
...
- **1.4 Soluția Propusă** (la nivel general) - o aplicație mobilă Flutter
  ...
  printr-un model mic, rulat pe dispozitiv: un Multi-Layer Perceptron
  (perceptron multistrat, MLP) cu arhitectura 22->32->16->1, antrenat prin
  Federated Learning cu păstrarea confidențialității (backend
  FastAPI/MongoDB/Redis).
- **1.5 Rezultate Obținute** (la nivel general) - aplicație funcțională cu
...
- **3.2 Abordări Centralizate vs. Federate pentru Recomandări** - filtrare
  colaborativă/bazată pe conținut cu date stocate pe server vs. FL;
  compromisuri legate de confidențialitate.
- **3.3 Federated Learning: Stadiul Actual** - taxonomia FL (Federated
  Learning Orizontal, Vertical și Federated Transfer Learning - FL cu
  transfer -, respectiv Cross-Device - la nivel de dispozitive individuale -
  versus Cross-Silo - la nivel de organizații) folosită pentru a poziționa
  CultureQuest ca **FL orizontal, de tip cross-device**; FedAvg, FL asincron,
  robustețe și confidențialitate (vezi Bibliografia de mai jos pentru
  articolele de tip survey care susțin această secțiune).
...
- **3.5 Stack-ul Tehnologic și Justificarea Acestuia**
  - 3.5.1 Flutter + Riverpod (vs. React Native/nativ)
  - 3.5.2 FastAPI + MongoDB + Redis (vs. alternative)
  - 3.5.3 OSRM pentru rutare
  - 3.5.4 MLP propriu vs. TFLite/PyTorch Mobile (dimensiunea modelului, fără
    dependență de runtime)
...
- **4.2 Arhitectura Aplicației Mobile** - structură Flutter organizată pe
  ...
  Riverpod.
- **4.3 Arhitectura Backend-ului** - servicii/rute FastAPI, modelul de date
  (obiective, utilizatori, quest-uri, rute, evenimente).
- **4.4 Proiectarea Sistemului de Federated Learning**
  ...
  - 4.4.4 Agregare cu Limitare (Clipping) (limitarea normei delta-ului
    împotriva actualizărilor adversariale/extreme)
  - 4.4.5 Confidențialitate Diferențială (Differential Privacy, DP) (zgomot
    Gaussian adăugat pe delta-urile ponderilor)
  - 4.4.6 Controlul Concurenței (lock de agregare în Redis)
- **4.5 Pipeline-ul de Generare a Rutelor** - filtrare cu scor FL x 0.8 +
...
- **5.1 Implementarea Aplicației Mobile** (subsecțiunile orientate spre
  ...
  - 5.1.2 Injectarea Zgomotului pentru Confidențialitate Diferențială (zgomot
    Gaussian generat prin transformata Box-Muller)
  - 5.1.3 Buffer-ul Local de Interacțiuni și Persistența Acestuia (cache
    bazat pe SharedPreferences, persistă la închiderea aplicației)
  - 5.1.4 Stocarea în Cache a Obiectivelor pentru Funcționarea Offline
  ...
  - 5.1.6 Sistemul de Quest-uri și Geofencing - detectarea finalizării
    quest-urilor, declanșatoare de geofencing, înregistrarea etichetelor de
    engagement
- **5.2 Implementarea Backend-ului**
  ...
  - 5.2.2 Endpoint-uri API (preluare/actualizare model, status)
- **5.3 Pipeline-ul de Pre-antrenare**
  - 5.3.1 Pregătirea Seturilor de Date (Foursquare TSMC2014/TIST2015,
    ubicomp2013, Yelp, date sintetice; construcția etichetelor)
  - 5.3.2 Antrenarea de la Zero (MLP, SGD, scăderea ratei de învățare)
  - 5.3.3 Fine-Tuning (augmentare de date pentru semnal slab de etichetare,
    modificări de hiperparametri, corectarea regresiei către medie)
- **5.4 Framework-ul de Simulare FL** - simulare sintetică pe mai multe runde,
  comparând reducerea în funcție de vechime din FedAsync activată vs.
  dezactivată, modelarea derivei conceptuale (concept drift) pentru clienții
  cu date vechi (stale).
- **5.5 Provocări Notabile de Implementare** - problema regresiei către medie
  din pre-antrenare și modul în care fine-tuning-ul a rezolvat-o; ajustarea
  reducerii în funcție de vechime.
...
- **6.2 Performanța Modelului FL**
  - 6.2.1 Rezultatele Pre-antrenării (eroare medie pătratică - Mean Squared
    Error, MSE -, R^2, intervalul predicțiilor)
  - 6.2.2 Rezultatele Fine-Tuning-ului (tabel comparativ înainte/după)
  - 6.2.3 Rezultatele Simulării (curbe de loss, deriva ponderilor, distribuția
    vechimii (staleness), probe contextuale pe parcursul a 600 de runde)
...
- **6.4 Comparație și Discuții** - compromisurile dintre FedAvg sincron și
  asincron (problema clienților întârziați - straggler problem); poziționarea
  față de alternativele din Capitolul 3.

## Capitolul 7 - Concluzii
- **7.1 Sumar** - obiectivele revizuite în raport cu ce a fost livrat.
- **7.2 Lucrări Viitoare** - "personal head" per utilizator, agregare de tip
  buffer (FedBuff) dacă participarea clienților se grupează în timp, schimbări
  de model/arhitectură mai ample, colectare de date FL de la utilizatori
  reali la scară mare.
...
| Xie, C., Koyejo, S., & Gupta, I. (2019). Asynchronous Federated Optimization. *arXiv:1903.03934*. | Reducerea pe bază de vechime (staleness discount) din FedAsync (`n_effective = n_samples/(1+staleness)`), nucleul agregării asincrone din CultureQuest. | 3.3, 4.4.3, 6.2.3 |
...
| Citare | Folosit pentru | Citat în |
|---|---|---|
| Yang, D., Zhang, D., Zheng, V. W., & Yu, Z. (2015). Modeling User Activity Preference by Leveraging User Spatial Temporal Characteristics in LBSNs. *IEEE Transactions on Systems, Man, and Cybernetics: Systems*, 45(1). | Setul de date TSMC2014 (check-in-uri Foursquare din New York și Tokyo). | 5.3.1 |
| Yang, D., Zhang, D., & Qu, B. (2016). Participatory Cultural Mapping Based on Collective Behavior Data in Location-Based Social Networks. *ACM Transactions on Intelligent Systems and Technology*, 7(3). | Setul de date TIST2015 (check-in-uri Foursquare la nivel global). | 5.3.1 |
| Yang, D., Zhang, D., Yu, Z., & Yu, Z. (2013). Fine-Grained Preference-Aware Location Search Leveraging Crowdsourced Digital Footprints from LBSNs. *UbiComp 2013*. | Setul de date ubicomp2013 (check-in-uri/recenzii la restaurante din New York). | 5.3.1 |
| Yelp Inc. Yelp Open Dataset. (academic license). | Recenzii Yelp - sursa etichetelor explicite cu sentiment negativ (recenzii de 1-2 stele). | 5.3.1 |
